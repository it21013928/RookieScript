...
function UserProfile() {
  ...
  const [lineChartData, setLineChartData] = useState({ scoreCount: 0, scoreList: [] });
...
useEffect(() => {
  Axios.get('api/user/getScoresList')
    .then(response => {
      ...
      const xLabels = Array.from({ length: scoreCount }, (_, index) => (index + 1).toString());
...
/////////////////////////////
useEffect(() => {
  Axios.get('api/user/getUserRank')
    .then(response => {
      console.log(response.data); // Access the response data here
    ...
    <div className="user-profile-container">
      ...
      <div className="right-side">
        <div className="user-leaderboard-card">
          ...
          <canvas id="lineChart" ref={chartRefLine} width={400} height={200}></canvas>
        </div>
